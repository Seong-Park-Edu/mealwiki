...
namespace Server.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> Signup([FromBody] AuthRequest request)
        {
            ...
            var existing = await _supabase.From<User>().Where(u => u.Nickname == request.Nickname).Get();
            if (existing.Models.Any()) return BadRequest("이미 존재하는 닉네임입니다.");
        ...
        public async Task<IActionResult> Login([FromBody] AuthRequest request)
        {
            try
            {
                ...
                if (user == null) return Unauthorized("닉네임 또는 비밀번호가 틀렸습니다.");
        ...
        {
            var response = await _supabase.From<User>().Where(u => u.Nickname == request.Nickname).Where(u => u.Email == request.Email).Get();
            var user = response.Models.FirstOrDefault();
            if (user == null) return NotFound("일치하는 회원 정보가 없습니다.");

            var tempPassword = new Random().Next(1000, 9999).ToString();
            await _supabase.From<User>().Where(u => u.Id == user.Id).Set(u => u.Password, tempPassword).Update();
        ...
        {
            var response = await _supabase.From<User>().Where(u => u.Nickname == request.Nickname).Where(u => u.Password == request.OldPassword).Get();
            var user = response.Models.FirstOrDefault();
            if (user == null) return BadRequest("현재 비밀번호가 일치하지 않습니다.");

            await _supabase.From<User>().Where(u => u.Id == user.Id).Set(u => u.Password, request.NewPassword).Update();
        ...
        {
            if (!Guid.TryParse(userId, out var userGuid)) return BadRequest("ID 형식 오류");
            ...
            try
            {
                ...
                var posts = (await _supabase.From<WikiPost>()
                    .Filter("author_id", Supabase.Postgrest.Constants.Operator.Equals, userGuid.ToString())
                    .Order("updated_at", Supabase.Postgrest.Constants.Ordering.Descending).Get()).Models;

                var acks = (await _supabase.From<UserAck>()
                    .Filter("user_id", Supabase.Postgrest.Constants.Operator.Equals, userGuid.ToString())
                    .Order("created_at", Supabase.Postgrest.Constants.Ordering.Descending).Get()).Models;

                var bookmarks = (await _supabase.From<UserBookmark>()
                    .Filter("user_id", Supabase.Postgrest.Constants.Operator.Equals, userGuid.ToString())
                    .Order("created_at", Supabase.Postgrest.Constants.Ordering.Descending).Get()).Models;

                // 3. ★ 모든 관련 식당 ID 수집
                var allRestaurantIds = posts.Select(p => p.RestaurantId)
                ...
                {
                    var rRes = await _supabase.From<Restaurant>()
                ...
                return Ok(new
                {
                    Profile = new { Nickname = user.Nickname, ReviewCount = user.ReviewCount, TotalLikes = user.TotalLikes, CreatedAt = user.CreatedAt },

                    Reviews = posts.Select(p => new
                    {
                        Id = p.Id,
                        RestaurantId = p.RestaurantId,
                        RestaurantName = restaurantMap.ContainsKey(p.RestaurantId) ? restaurantMap[p.RestaurantId].Name : "알 수 없음",
                        Content = p.Content,
                    ...
                    Acks = acks.Select(a => new
                    {
                        ...
                        RestaurantName = restaurantMap.ContainsKey(a.RestaurantId) ? restaurantMap[a.RestaurantId].Name : "식당",
                    ...
                    {
                        var r = restaurantMap.ContainsKey(b.RestaurantId) ? restaurantMap[b.RestaurantId] : null;
                        return new
                        {
